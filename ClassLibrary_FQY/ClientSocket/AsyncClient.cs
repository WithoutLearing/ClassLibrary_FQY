...
namespace ClassLibrary_FQY.ClientSocket
{
    public class AsyncClient
    {
        ...
        /// <param name="ip">服务器ip</param>
        /// <param name="port">服务器端口</param>
        /// <param name="locip">绑定本地ip</param>
        public void StartClient(string ip, int port, string locip)
        {
            try
            {
                //为套接字建立远程端点
                ...
                Socket client = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
                //绑定本地IP
                client.Bind(new IPEndPoint(IPAddress.Parse(locip), 0));
                // Connect to the remote endpoint.  
                client.BeginConnect(remoteEP, new AsyncCallback(ConnectCallback), client);
        ...
        /// <returns>返回发送的数据个数</returns>
        public int SendAsync(string strData)
        {
